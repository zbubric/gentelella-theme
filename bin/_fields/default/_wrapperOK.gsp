...
%>
    <div class="item form-group ${invalid ? 'bad' : ''}">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
            <!-- <gent:widget bean="${bean}" property="${property}" widget="${widgetFolder}" class="${widgetClazz}" data-inputmask="${widgetMask}" /> -->
            <%=widget%>
        </div>
        <>
        ${raw(fieldIconArea)}
        <g:if test="${fieldIconLeft}">
                Hello Fred!
            </g:if>
            <g:elseif test="${fieldIconLeft}">
                Hello bob!
            </g:elseif>        
        <g:if test="${invalid}">
